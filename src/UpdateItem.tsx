...
const UpdateMenuItemForm: React.FC<UpdateMenuItemFormProps> = ({
  ...
  const validateForm = () => {
    ...
    if (!allergens) {
      errors.allergens = 'Allergens are required';
    }
    ...
    if (!cookingInstructions) {
      errors.cookingInstructions = 'Cooking Instructions are required';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.append('cookingInstructions', cookingInstructions);
...
const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  if (file) {
    ...
    setImage(null);
    setImagePreview(null); // Reset imagePreview when no image is selected
  }
};
...
{/* Allergens */}
<div className="mb-4">
  <label htmlFor="allergens" className="block text-sm font-medium">
    Allergens
  </label>
  <input
    id="allergens"
...
<div className="mb-4">
  ...
  />
  {errors.servingSize && (
    <p className="text-red-500 text-sm mt-1">{errors.servingSize}</p>
  )}
</div>
...
<div className="mb-4">
  <label htmlFor="cookingInstructions" className="block text-sm font-medium">
    Cooking Instructions
  </label>
  <textarea
    ...
    className={`
      ...
      ${isDarkMode ? 'bg-gray-600 text-white' : 'bg-gray-300 text-black'}
      mt-2
      ${errors.cookingInstructions ? 'border-red-500' : 'border-gray-300'}
      rounded-lg
      shadow-sm
      focus:border-blue-500
      focus:outline-none
  ...
  {errors.cookingInstructions && (
    <p className="text-red-500 text-sm mt-1">{errors.cookingInstructions}</p>
  )}
</div>

{/* Dietary Restrictions */}
<div className="mb-4">
  <label htmlFor="dietaryRestrictions" className="block text-sm font-medium">
    Dietary Restrictions
  </label>
  <input
  ...
  {errors.dietaryRestrictions && (
    <p className="text-red-500 text-sm mt-1">{errors.dietaryRestrictions}</p>
  )}
</div>
